...
public class Test_Snake extends Actor{
	private static final int SNAKE_START_LENGTH = 5;
	...
	public void act(){
		...
		updateSnakeBody();	// update snake body

		checkSpeedTimer();	// update timer
	    
		if (isMoveValid()) {	// check for valid move
		  int lastIndex = tailList.size() - 1;
			...
			if (checkForSnakeBody()) {
				...
				gameOver();	// Collision with SnakeBody :(
			}
				if (checkForApple()) {
				appleEaten++;
	...
	/**
	 * Moves the SnakeBody to the new position. SHows snake grow when collects an apple
	 */
	 ...
	 */
	public boolean checkForSnakeBody() {
	...
	private void gameWon() {
		gameGrid.setTitle("GRATULATION (Score: " + countScore + " | Apples: " + appleEaten + " | Speed: " + speedPercent + "%)");
		gameGrid.doPause();
		gameFinish = true;
	}
